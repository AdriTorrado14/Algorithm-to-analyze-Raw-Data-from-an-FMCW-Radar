...
def curveFitting_sineFunction(l_period_analyze_Time, subtract_l_full_range, valor):
    tt = np.array(l_period_analyze_Time[valor])
    yy = np.array(subtract_l_full_range[valor])
    ...
    #print(freq_SineFunction_Fitting_R)
...
"""
def calculation(peaks_Time_B, peaks_TimeB_1m_PC, size_list_intersect, peaks_Time_A, frequency_estimation_ZC):
    a1 = peaks_Time_B
    #print(len(a1))
    ...
    for t in range(len(size_list_intersect)):
        if (len(peaks_Time_B) <= 29 and len(peaks_TimeB_1m_PC) <= 15 and (size_list_intersect[t]) <= 5):
            ...
            return(l_distance_periods, average_distance)
...
def procesado_conjunto_Average():
    ...
    for i in range(len(l_filename)):
        ...
        curveFitting_sineFunction(periods_Time, corrected_Signals_periods, valor)

        fft_method(periods_channelB, samples, valor)

        (l_distance_periods, average_distance) = calculation(peaks_Time_B, peaks_Time_B_PC, size_list_intersect, peaks_Time_A, frequency_estimation_ZC)

        l_distances_aux.append(l_distance_periods) #ALL THREE PERIODS
        l_distances_aux_average.append(average_distance) #AVERAGE

    ######STANDARD DEVIATION --> AMPLIAR.
...
"""
(Time, channelA, channelB) = read_csv(file10)
...
#procesado_selectivo_instant_andRepresentation(file4)
