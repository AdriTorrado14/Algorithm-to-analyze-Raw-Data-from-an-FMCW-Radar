...
"""
def samples_IntervalTime(timeReal, actualSamplingInterval):
    #First part (Samples and period).
    intervalTime = abs(timeReal[10]-timeReal[9]) #In ms.
    #print("intervalTime")
    #print(intervalTime)
    #samples = T_halfPeriod/intervalTime
    samples = T_halfPeriod/actualSamplingInterval
...
def peaks_section_channel(timeReal, channelA, channelB):
    peaks_A, _ = find_peaks(channelA, height = 7.93) #Channel A
    l_peaks_A = []
    ...
    ### ADD 4 TO A SINGLE SAMPLE ###
    l_peaks_mod = []
    for i in range(len(l_peaks_A_indexSamples)):
        l_peaks_mod.append(l_peaks_A_indexSamples[i] + 3)
    #print(l_peaks_mod)
            
    ######WE USE "l_peaks_A_indexSamples" TO GET THE MEASUREMENT.
    peaks_Time_A = []
    for s in l_peaks_mod:
...
def interpolation_calculus_2part(output_subtract_div, output_Time_div):
    ...
    #output_interpolation = [list(int(islice(Input_interpolation, elem))) for elem in union_div]
...
"""
Functionality: Sinusoid function.
Input: 
Output: 
"""
